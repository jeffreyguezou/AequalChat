...
const Optionnav = () => {
  ...
  useEffect(() => {
    if (appData[0]) {
      reqData = appData[0].requests;
      let numberOfReqs = reqData.length;
      setRequestsNo(numberOfReqs);
    }
  }, [appData]);

  const setacuurentTab = (val: string) => {
    setActiveTab(val);
  };
    ...
    <div className="mb-auto">
      <div className="dark:bg-slate-600 flex h-14 bottom-0">
        ...
        >
          ...
          {requestsNo !== 0 && (
            <div className="bg-green-400 h-6 w-6 rounded-full">
              {requestsNo}
            </div>
          )}
        </div>
